	...
	app.controller('DailyReportController', ['$scope', 'netManager',
		function($scope, netManager) {
			...
			if(moment().format('YYYY-MM-DD HH:mm:ss') < moment(time).format('YYYY-MM-DD HH:mm:ss')) {
				$scope.dateData.searchDate = moment().subtract(2, 'days').format('YYYY-MM-DD HH:mm:ss')
			} else {
				$scope.dateData.searchDate = moment().subtract(1, 'days').format('YYYY-MM-DD HH:mm:ss')
			};
			...
			function getData(checkDate) {
				$scope.isLoad = true;
				var sendData = {
					time: checkDate ? checkDate : moment($scope.dateData.searchDate).format('YYYY-MM-DD')
				};
		...
		function($scope) {
			$scope.$on('resData', function(event, data) {
				...
				for(var i = 0; i < resData.length; i++) {
					salesPercent.push(resData[i].dailySales.toFixed(2));
					countsPercent.push(resData[i].dailyCount.toFixed(2));
