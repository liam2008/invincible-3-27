	...
	app.controller('DailyListController', ['$scope', 'DTOptionsBuilder', 'netManager', '$filter',
		function($scope, DTOptionsBuilder, netManager, $filter) {
			...
			if(moment().format('YYYY-MM-DD HH:mm:ss') < moment(time).format('YYYY-MM-DD HH:mm:ss')) {
				$scope.dateData.startDate = moment().subtract(2, 'days').format('YYYY-MM-DD');
				$scope.dateData.endDate = moment().subtract(2, 'days').format('YYYY-MM-DD');
			} else {
				$scope.dateData.startDate = moment().subtract(1, 'days').format('YYYY-MM-DD');
				$scope.dateData.endDate = moment().subtract(1, 'days').format('YYYY-MM-DD');
			};
