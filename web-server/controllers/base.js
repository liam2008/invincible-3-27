...
module.exports = {
	...
	list: function(req, res, next) {
		...
		if(subFilter.view == "*") {
			...
			switch(subFilter.view[0]) {
				case "team":
					{
						findCondition = {
							team_id: subFilter.view[1]
						};
						break;
					}
				case "manager":
		...
		if(subFilter.edit) {
			Merchandise.findOne({
				...
				if(MerchandiseResult) {
					...
					if(manager && (MerchandiseResult.manager == null || manager.toString() != MerchandiseResult.manager.toString())) {
						needSave = true;
						changes.manager = {
							from: MerchandiseResult.manager,
							to: manager
						};
						MerchandiseResult.manager = mongoose.Types.ObjectId(manager);
					}
					let productId = req.body.productId;
					if(productId && (MerchandiseResult.product_id == null || productId.toString() != MerchandiseResult.product_id.toString())) {
						needSave = true;
						changes.productId = {
							from: MerchandiseResult.product_id,
							to: productId
						};
						MerchandiseResult.product_id = mongoose.Types.ObjectId(productId);
					}
					let shelfTime = req.body.shelfTime;
					if(shelfTime && (MerchandiseResult.shelf_time == null || moment(shelfTime).format('YYYY-MM-DD') != moment(MerchandiseResult.shelf_time).format('YYYY-MM-DD'))) {
						needSave = true;
						MerchandiseResult.shelf_time = MerchandiseResult.shelf_time || "";
						changes.shelfTime = {
							from: moment(MerchandiseResult.shelf_time).format('YYYY-MM-DD'),
							to: moment(shelfTime).format('YYYY-MM-DD')
						};
						MerchandiseResult.shelf_time = moment(shelfTime).format('YYYY-MM-DD');
					}
					let asin = req.body.asin;
					if(asin && (MerchandiseResult.asin == null || asin != MerchandiseResult.asin)) {
						needSave = true;
						changes.asin = {
							from: MerchandiseResult.asin,
							to: asin
						};
						MerchandiseResult.asin = asin;
					}
					let price = req.body.price;
					if(price && (MerchandiseResult.price == null || price != MerchandiseResult.price)) {
						needSave = true;
						changes.price = {
							from: MerchandiseResult.price,
							to: price
						};
						MerchandiseResult.price = parseInt(price);
					}
					let state = req.body.state;
					if(state && (MerchandiseResult.state == null || state != MerchandiseResult.state)) {
						needSave = true;
						changes.state = {
							from: MerchandiseResult.state,
							to: state
						};
						MerchandiseResult.state = parseInt(state);
					}
					let projectedSales = req.body.projectedSales;
					if(projectedSales && (MerchandiseResult.projected_sales == null || projectedSales != MerchandiseResult.projected_sales)) {
						needSave = true;
						changes.projectedSales = {
							from: MerchandiseResult.projected_sales,
							to: projectedSales
						};
						MerchandiseResult.projected_sales = parseInt(projectedSales);
					}
					let teamId = req.body.teamId;
					if(teamId && (MerchandiseResult.team_id == null || teamId.toString() != MerchandiseResult.team_id.toString())) {
						needSave = true;
						MerchandiseResult.team_id = MerchandiseResult.team_id || "";
						changes.teamId = {
							from: MerchandiseResult.team_id,
							to: teamId
						};
						MerchandiseResult.team_id = mongoose.Types.ObjectId(teamId);
					}
					let fnsku = req.body.fnsku;
					if(fnsku && (MerchandiseResult.fnsku == null || fnsku != MerchandiseResult.fnsku)) {
						needSave = true;
						changes.fnsku = {
							from: MerchandiseResult.fnsku,
							to: fnsku
						};
						MerchandiseResult.fnsku = fnsku;
					}

					if(needSave) {
						MerchandiseResult.updatedAt = moment().format('YYYY-MM-DD HH:mm:ss');
						// 修改日志
						dataUtils.historyDeal(req, MerchandiseResult, changes);
		...
		if(subFilter.add) {
			Merchandise.findOne({
				...
				if(!MerchandiseResult) {
					...
					res.error(ERROR_CODE.MERCHANDISE_ALREADY_EXISTS);
				}
			})
		} else {
			res.success();
