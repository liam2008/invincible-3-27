...
module.exports = {
	...
	report: function(req, res, next) {
		...
		var findRequire2 = {};
		if(teamRequire) {
			findRequire = teamRequire;
			findRequire2 = {
				$or: []
			};
		}
		debug("xxx: %j", findRequire)
	...
	operationSales: function(req, res) {
		...
		async.series([
			...
			function(callback) {
				...
				if (subFilter.viewInfo) {
				    switch (subFilter.viewInfo[0]) {
                        case "team": {
                            findWhere.team_id = subFilter.viewInfo[1];
                            break;
                        }
                        case "manager": {
                            findWhere.manager = subFilter.viewInfo[1];
                            break;
                        }
                    }
                }
