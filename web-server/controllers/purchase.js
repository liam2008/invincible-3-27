		...
		Purchase.findOne(findRequire, function(err, purOne) {
			...
			if(!purOne && skuNotRepeat) {
				...
				var iterator = function(json, cb) {
					...
					var unitPrice = 0;
					if(json.unitPrice) unitPrice = parseInt(json.unitPrice);
					var purQuantity = 0;
					if(json.purQuantity) purQuantity = parseInt(json.purQuantity);
					var totalPrice = 0;
					if(json.totalPrice) totalPrice = parseInt(json.totalPrice);
					purTotalPrice += totalPrice;
					var delQuantity = 0;
					if(json.delQuantity) delQuantity = parseInt(json.delQuantity);
					// 不为负数
					...
					async.series([
							...
							function(callB) {
								...
								if(json.delQuantity && json.deliverDate) {
									newPurDetail.deliver_date.push(json.deliverDate); // 实际交期
									newPurDetail.deliver_quantity.push(delQuantity); // 交货数量
								};
				...
				if(purOneFound) {
					...
					for(var m = 0; m < purOneFound.purDetails.length; m++) {
						...
						var purDetail = {
							...
							"delBatches": detFound.in_batches,
												...
												if(detListFound) {
													...
													if(req.body.orderStatus == 10) {
														...
														if(json.delQuantity && json.deliverDate) {
															...
															detFound.in_batches = json.delBatches || [];
														}
													}
		...
		Purchase.findOne(findRequire)
			...
			.exec(function(err, purOneFound) {
				...
				for(var m = 0; m < purOneFound.purDetails.length; m++) {
					...
					var delBatches = detFound.in_batches;
					if(!delBatches.length) {
						delBatches = [{
							number: detFound.purchase_quantity || 0,
							time: moment(detFound.contract_covenant_date).format('YYYY-MM-DD')
						}]
					};
					var purDetail = {
						"storeSku": detFound.store_sku,
						...
						"delPassage": 0,
						"delStorage": 0,
						"delBatches": delBatches,
						"remarks": detFound.remarks,
						"salesman": detFound.salesman || "",
						"logNumber": detFound.logistics_number || ""
					};

					for(var i = 0; i < detFound.deliver_quantity.length; i++) {
						var delQuantity = detFound.deliver_quantity[i];
						var delWarehouse = detFound.in_warehouse[i];
						if(delWarehouse) {
							purDetail.delStorage += delQuantity
						} else {
							purDetail.delPassage += delQuantity
						}
					}
					purDetail.delProduce = purDetail.purQuantity - purDetail.delStorage - purDetail.delPassage;
									...
									if(detOneFound) {
										...
										if(json.deliverDate && json.delQuantity) {
											...
											for(var m = 0; m < detOneFound.deliver_date.length; m++) {
												...
												InWarehouse.push(detOneFound.in_warehouse[m])
											};
											detOneFound.deliver_date = deliverDate;
											detOneFound.deliver_quantity = DELVRQuantity;
											detOneFound.in_warehouse = InWarehouse;
											detOneFound.deliver_total += delQuantity;
										} else {
											// 部分入库
											if(req.body.orderStatus == '50') {
												var InWarehouse = [];
												detOneFound.in_warehouse.map(function(item) {
													InWarehouse.push(item)
												});
												InWarehouse[0] = true;
												detOneFound.in_warehouse = InWarehouse;
											} else {
										...
										if(detOneFound.purchase_quantity >= detOneFound.deliver_total) {
											detOneFound.save(function(err, doc) {
												if(dealErr.updateErr(err, res)) return callB(err);
												callB(null);
											})
										} else {
