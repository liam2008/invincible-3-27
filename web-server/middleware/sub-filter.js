...
module.exports = {
	...
	merchandise_edit: function(req, res, next) {
		...
		switch(role.type) {
			...
			case "member":
				{
					result.view = ["manager", agent.id] || null;
					result.edit = true;
					result.add = true;
					result.delete = false;
					break;
			...
			case "trainingSupervisor":
				{
					result.view = ["team", "5a12305b569e8d470f884019"];
					result.edit = false;
					result.add = false;
					result.delete = false;
					break;
	...
	operationSales: function(req, res, next) {
		...
		switch(role.type) {
			...
			case 'leader':
				result.view = true;
				result.viewInfo = ["team", team.id];
				break;
				// 专员查看自己
			case 'member':
				result.view = true;
				result.viewInfo = ["manager", agent.id];
				break;
			default:
				result.view = null;
				break;
