...
AssetsManager.getWorkUrl = function(uuid, suffix, complete) {
	var url = envPath + "/work/" + uuid + "." + suffix;
	if(complete) {
		return this.uri + url;
	}
	return url;
}
AssetsManager.putWorkFile = function(file, callback) {
	var self = this;
	var data = file.data;
	var type = file.type;
	var uuid = file.uuid;
	var uri = self.getWorkUrl(uuid, type);

	client.put(uri, new Buffer(data, 'base64'))
